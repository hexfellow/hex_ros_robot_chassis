...
│   ├── utility/                         #   Trigger A3 LR1 DataInterface
...
├── setup.py                             # Python packaging (2 entry_points)
...
`utility/` (Trigger A3 LR1) and `maver_util/` (Maver) each provide a unified `DataInterface` that automatically selects the ROS 1 or ROS 2 implementation based on the `ROS_VERSION` environment variable:
...
> `maver_util/` has the same structure as `utility/` and is the Maver-specific interface layer.
...
| Publish | `odom` | `nav_msgs/(msg/)Odometry` | Odometry (position x, y, yaw + velocity vx, vy, omega) |
| Publish | `joint_states` | `sensor_msgs/(msg/)JointState` | 8 joint states (`joint_yaw1` ~ `joint_wheel4`) |
| Publish | `/tf` | `tf2_msgs/(msg/)TFMessage` | odom → base_link transform |

> **Joint order**: The 8 joints of Maver X4 (array indices 0~7) are ordered `joint_yaw1, joint_wheel1, joint_yaw2, joint_wheel2, joint_yaw3, joint_wheel3, joint_yaw4, joint_wheel4`; indices 0, 2, 4, 6 are steering (yaw) joints and indices 1, 3, 5, 7 are drive (wheel) joints. Both the `jnt` array of `chs_ctrl` and `joint_states` must follow this order.
...
> **Maver** current firmware forces `kp=0`; in MIT mode, only the feedforward torque (`eff`) and damping (`kd`) take effect.
...
Optional arguments for `maver.launch.py`:
- `rviz:=true/false`: whether to launch rviz visualization (default `true`, requires `hex_ros_urdf_maver_x4`)
...
Use `ros2 topic pub` to quickly send control commands (the 8 motors are ordered `joint_yaw1` ~ `joint_wheel4`):
...
# MIT mode — release (all zero, no output torque)
rostopic pub --once /chs_ctrl hex_ros_msgs/msg/HexRosRoboChsCtrlStamped "{header: {stamp: 0, frame_id: 'base_link'}, chs_ctrl: {ctrl_mode: 1, jnt: {pos: [0, 0, 0, 0, 0, 0, 0, 0], vel: [0, 0, 0, 0, 0, 0, 0, 0], eff: [0, 0, 0, 0, 0, 0, 0, 0], kp: [0, 0, 0, 0, 0, 0, 0, 0], kd: [0, 0, 0, 0, 0, 0, 0, 0], lim_vel: [], lim_acc: []}, vel: {linear: {x: 0.0, y: 0.0, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 0.0}}}}"
```
...
| `jnt.eff[0..7]` | Feedforward torque | Nm |
...
> The array order of the 8 motors is `joint_yaw1, joint_wheel1, joint_yaw2, joint_wheel2, joint_yaw3, joint_wheel3, joint_yaw4, joint_wheel4` (indices 0, 2, 4, 6 = yaw steering, 1, 3, 5, 7 = wheel drive).
