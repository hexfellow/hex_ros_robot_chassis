...
│   ├── utility/                         #   Trigger A3 LR1 的 DataInterface
...
│       └── ros2_interface.py            #     ROS 2 DataInterface 实现
├── resource/                            # ament 资源文件
├── setup.py                             # Python 打包配置（2 个 entry_point）
...
`utility/`（Trigger A3 LR1）与 `maver_util/`（Maver）模块各提供统一的 `DataInterface`，根据 `ROS_VERSION` 环境变量自动选择 ROS 1 或 ROS 2 实现：
...
> `maver_util/` 结构与 `utility/` 相同，为 Maver 专用接口层。
...
> **Maver**当前固件强制 `kp=0`，MIT 模式实际生效的是前馈力矩（`eff`）与阻尼（`kd`）。
...
`maver.launch.py` 可选参数：
- `rviz:=true/false`：是否启动 rviz 可视化（默认 `true`，需要 `hex_ros_urdf_maver_x4`）
...
# MIT 模式 — 纯力矩前馈（8 个电机各 1.0 Nm，其余为 0）
ros2 topic pub --once /chs_ctrl hex_ros_msgs/msg/HexRosRoboChsCtrlStamped \
